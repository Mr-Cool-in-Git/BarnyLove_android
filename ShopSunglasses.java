...
public class ShopSunglasses extends State implements Screen {

    Texture fon, ClSunG, ClWith, butbuy, usebut, usedbut;
    ...
    public ShopSunglasses(final GameStateManager gam) {
        super(gam);
        ...
        GlassesStatus = Gdx.app.getPreferences("ShopStatus");
        NumberGlasses = GlassesStatus.getString("Glasses", "0");
        BuySG = GlassesStatus.getString("BuySG", "0");

        StatusBuySG = Integer.valueOf(BuySG);
        StatusUse = Integer.valueOf(NumberGlasses);
        ...
        ClSunG = new Texture("sunglasses.png");
        ClWith = new Texture("without.png");
        fon = new Texture("dutyfreesunglasses.png");
        ...
        B1.addListener(new ChangeListener() {
            @Override
            public void changed(ChangeEvent event, Actor actor) {  if ((StatusBuySG == 1) && (StatusUse == 1)){
                StatusUse = 0;
                GlassesStatus.putString("Glasses" , Integer.toString(StatusUse));
                GlassesStatus.flush();
            }}
        });
        B2.addListener(new ChangeListener() {
            @Override
            public void changed(ChangeEvent event, Actor actor) {if ((StatusBuySG == 1) && (StatusUse == 0)){
                StatusUse = 1;
                GlassesStatus.putString("Glasses" , Integer.toString(StatusUse));
                GlassesStatus.flush();
            }
                if (StatusBuySG == 0){
                    ...
                    if (MyCash >= 350){
                        ...
                        MoneyStatus = Money.getString("Money", "0");
                        StatusBuySG = 1;
                        GlassesStatus.putString("BuySG", Integer.toString(StatusBuySG));
                        GlassesStatus.flush();
                    }
                };}
        });
    ...
    public void render(SpriteBatch sb) {
        ...
        sb.draw(ClSunG, 250, 450);
        if ((StatusBuySG == 0) && (StatusUse == 0)) {
            sb.draw(butbuy, 255, 355);
            sb.draw(usedbut, 55, 355);
        }
        if ((StatusBuySG == 1) && (StatusUse == 0)) {
            sb.draw(usebut, 255, 355);
            sb.draw(usedbut, 55, 355);
        }
        if ((StatusBuySG == 1) && (StatusUse == 1)) {
            sb.draw(usedbut, 255, 355);
            sb.draw(usebut, 55, 355);
        }
        if (StatusBuySG == 0) {
            price.setColor(0, 0, 0, 1);
            price.draw(sb, "Price: 350", 238, 330);
        }
